...
bounding_boxes *GetBoundingBoxesVerbose(TessBaseAPI a) {
  ...
  while (!res_it->Empty(RIL_BLOCK)) {
    ...
    if (box_array->length >= realloc_threshold) {
      ...
      box_array->boxes = (bounding_box *)realloc(
          box_array->boxes, capacity * sizeof(bounding_box));
      realloc_threshold += realloc_raise;
    }

    box_array->boxes[box_array->length].word = res_it->GetUTF8Text(RIL_WORD);
    box_array->boxes[box_array->length].confidence =
        res_it->Confidence(RIL_WORD);
    res_it->BoundingBox(RIL_WORD, &box_array->boxes[box_array->length].x1,
...
bounding_boxes *GetBoundingBoxes(TessBaseAPI a, int pageIteratorLevel) {
  ...
  tesseract::PageIteratorLevel level =
      (tesseract::PageIteratorLevel)pageIteratorLevel;
  ...
  if (ri != 0) {
    do {
      ...
      ri->BoundingBox(level, &box_array->boxes[box_array->length].x1,
                      &box_array->boxes[box_array->length].y1,
                      &box_array->boxes[box_array->length].x2,
                      &box_array->boxes[box_array->length].y2);
      box_array->length++;
